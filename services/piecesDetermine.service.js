...
export const piecesDetermine = {
  ...
  generateDeterminations(pieceBoxPositionsObject = null) {
    ...
    for (const {
      ...
      isWhitePiece,
      pieceBoxPosition,
    } of pieceBoxPositionsObject || this.getCurrentPieceBoxPositions()) {
  ...
  itereateDetermine(pieceBoxPosition, callbackFn) {
    ...
    if (!determinationPositions || !Object.keys(determinationPositions).length)
      return;
  ...
  hasPiecePotential(pieceBoxPosition, determinationPosition) {
    return (
      this.determinations[pieceBoxPosition]?.[determinationPosition] ?? false
    );
  },
