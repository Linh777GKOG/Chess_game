...
export const checkMate = {
  ...
  cantMoveDueToCheck({ pieceSelectedPosition, pieceBoxPosition }) {
    ...
    let kingPiecePosition = checkMate.getKingPiecePosition({
    ...
    const pieceBoxPositionsObject = Object.keys(piecesDetermine.determinations)
    ...
    const result = [
      ...new Set(
        Object.values(piecesDetermine.determinations)
          .map((determinations) => Object.keys(determinations))
          .flat()
      ),
  ...
  isCheckMate() {
    ...
    this.gameOver =
      Object.keys(piecesDetermine.determinations)
        ...
        .map((sourcePieceBoxPosition) => {
          return Object.keys(
            piecesDetermine.determinations[sourcePieceBoxPosition]
  ...
  displayCheckMateMessage() {
    playerTurn.isWhiteTurn ? this.blackWinsMessage() : this.whiteWinsMessage();
  },
  blackWinsMessage() {
